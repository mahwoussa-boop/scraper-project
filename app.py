...
def render_pro_table(df, prefix, section_type="update", show_search=True):
    ...
    for idx, row in page_df.iterrows():
        ...
        # استخراج الصور للمقارنة البصرية
        our_img = str(row.get("رابط_الصورة", row.get("image_url", "")))
        comp_img = ""
        all_comps = row.get("جميع_المنافسين", row.get("جميع المنافسين", []))
        if isinstance(all_comps, list) and len(all_comps) > 0:
            # ابحث عن صورة المنافس المتصدر (الأقل سعراً)
            sorted_comps = sorted(all_comps, key=lambda c: safe_float(c.get("price", 0)))
            comp_img = str(sorted_comps[0].get("image_url", ""))

        # بطاقة VS مع رقم المنتج والصور
...
# ════════════════════════════════════════════════
    ...
    st.subheader("🏪 كشط مواقع المنافسين")
    
    # قائمة المنافسين المزودة من المستخدم
    COMPETITORS_LIST = {
        "سعيد صلاح": "https://saeedsalah.com/",
        "فانيلا (Vanilla)": "https://vanilla.sa/",
        "سارا ميكب": "https://sara-makeup.com/",
        "خبير العطور": "https://alkhabeershop.com/",
        "بسمة": "https://basmaperfumes.com/",
        "لي سانتو (Le Santo)": "https://leesanto.com/",
        "آزال (Azal)": "https://azalperfume.com/",
        "كاندي نيش": "https://candyniche.com/",
        "الفاخرة للنيش": "https://luxuryperfumesnish.com/",
        "حنان العطور": "https://hanan-store55.com/",
        "اريج امواج": "https://areejamwaj.com/",
        "نيش (Niche)": "https://niche.sa/",
        "عالم جيفنشي": "https://worldgivenchy.com/",
        "ساره ستور": "https://sarahmakeup37.com/",
        "اروماتيك كلاود": "https://aromaticcloud.com/",
        "نايس ون (Nice One)": "https://niceonesa.com/",
        "سيفورا (Sephora)": "https://www.sephora.me/sa-ar",
        "وجوه (Faces)": "https://www.faces.sa/ar",
        "تطيّب (Tatayab)": "https://tatayab.com/",
        "كيان (Kayan)": "https://kayan9.com/"
    }

    selected_comps = st.multiselect("اختر المنافسين للكشط", list(COMPETITORS_LIST.keys()), key="comp_select")
    
    # توليد روابط Sitemap تلقائياً للمنافسين المختارين (بناءً على نمط سلة وزد)
    auto_sitemaps = []
    for c in selected_comps:
        base_url = COMPETITORS_LIST[c].rstrip('/')
        # معظم هذه المتاجر على سلة/زد وتستخدم هذا المسار
        auto_sitemaps.append(f"{base_url}/sitemap.xml")
    
    sitemap_input = st.text_area("روابط Sitemap للمنافسين (يمكنك التعديل أو إضافة روابط يدوية)", 
                                 value="\n".join(auto_sitemaps),
    ...
    if st.button("🚀 بدء الكشط والتحليل", type="primary"):
        if our_file and sitemap_input.strip():
            ...
            if err:
                ...
                scraping_prog = st.progress(0)
                
                def update_scraping(msg, p):
                    scraping_status.info(msg)
                    scraping_prog.progress(p)
                ...
                # تحميل الملف المكشوط
                comp_latest_path = "data/competitors_latest.csv"
                if os.path.exists(comp_latest_path):
                    ...
                    st.error("❌ فشل العثور على البيانات المكشوطة")
        else:
            st.warning("⚠️ يرجى رفع ملف منتجاتنا وإدخال رابط Sitemap واحد على الأقل")
    ...
    if st.session_state.results and "missing" in st.session_state.results:
        ...
        if df is not None and not df.empty:
            ...
            for idx, row in page_df.iterrows():
                ...
                # عرض صورة المنتج المفقود
                miss_img = str(row.get("رابط_الصورة", row.get("image_url", "")))
